...
ledPitch = 4
#acc = ((9.8 * 1000) / ledPitch) * tickSpeed
acc = -0.005
maxInitSpeed = 0.55
#acc = -0.06
#maxInitSpeed = 2
...
class Drop:
    def __init__(self):
        ...
        self.generateColor()
        #self.speed = 1 + (random.random() * maxInitSpeed)
        self.speed = random.uniform(0, maxInitSpeed)
    ...
    def generateColor(self):
        ...
        if (mode == 0):
            (r, g, b) = colorsys.hsv_to_rgb((float(self.x) / (length - 1)), 1, 1)
...
def main():
    ...
    while (True):
        for j in range(0, len(drops)):
            drops[j].tick()
            if drops[j].strength == 0 or drops[j].y == (rows - 1):
               drops[j].erase()
               drops[j] = Drop()
