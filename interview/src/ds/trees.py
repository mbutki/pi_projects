...
class BiSearchTree:
    ...
    def insert_iter(self, node: Node) -> None:
    ...
    def insert_recur(self, node: Node) -> None:
        if self.root is None:
            self.root = node
            return
        self.insert_recur_helper(self.root, node)

    def insert_recur_helper(self, cur_node: Node, node: Node) -> None:
        if node <= cur_node:
            left_child = cur_node.get_left_child()
            if left_child is None:
                cur_node.set_left_child(node)
    ...
    def delete_recur(self, node: Node) -> None:
        if self.root is None:
            return
    ...
    def delete_recur_helper(
        self, cur_node: Node, cur_parent: Node | None, node: Node
    ) -> None:
        if node < cur_node:  # just trying to find the node
            ...
            ):  # find right most child of direct left child, copy value up, call recursive on that child
                largest_child, largest_child_parent = self.find_largest_child(
                    left_child
                )
                cur_node.set_name(largest_child.get_name())
                self.delete_recur_helper(largest_child, largest_child_parent, node)
            elif left_child:  # only 1 child, bypass current node
                if cur_parent.get_left_child() == cur_node:
                    cur_parent.set_left_child(cur_node.get_left_child())
                else:
                    cur_parent.set_right_child(cur_node.get_left_child())
            elif right_child:  # only 1 child, copy child value up, delete child
                if cur_parent.get_left_child() == cur_node:
                    cur_parent.set_left_child(cur_node.get_right_child())
                else:
                    cur_parent.set_right_child(cur_node.get_right_child())
    ...
    def find_largest_child(self, node: Node) -> tuple[Node, Node | None]:
        parent = None
    ...
    def dfs_recur_pre(self, node: None, goal: str) -> None:
        if node is None:
            return
        print(node.get_name())
    ...
    def dfs_recur_in(self, node: None, goal: str) -> None:
        if node is None:
            return
    ...
    def dfs_recur_post(self, node: None, goal: str) -> None:
        if node is None:
            return
    ...
    def dfs_iter_pre(self, _goal: str) -> bool:
        stack = [self.root]
        while len(stack) != 0:
            node = stack.pop()
            if node is None:
    ...
    def dfs_iter_in(self, _goal: str) -> bool:
        stack = []
        cur_node = self.root
    ...
    def dfs_iter_post(self, _goal: str) -> bool:
        stack = [self.root]
    ...
    def bfs_iter(self, _goal: str) -> None:
        queue = [self.root]
        while len(queue) != 0:
            node = queue.pop(0)
            print(node.get_name())
